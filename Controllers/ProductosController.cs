    ...
    IConfiguration configuration) : Controller
{
    ...
    public async Task<IActionResult> EditarAsync(int id, Producto itemToEdit)
    {
        ...
        await ProductosDropDownListAsync();
        ModelState.AddModelError("Nombre", "No ha sido posible realizar la acción. Intentelo de nuevo.");
        return View(itemToEdit);
    }
    ...
    public async Task<IActionResult> Eliminar (int id, bool? showError = false)
    {
        ...
        try
        {
            ...
            if(showError.GetValueOrDefault())
                ViewData["ErrorMessage"] = "No ha sido posible realizar la acción. Intentelo mas tarde";
        }
        catch (HttpRequestException ex)
        {
            if(ex.StatusCode == System.Net.HttpStatusCode.Unauthorized)
        ...
        ViewBag.Url = configuration["WebUrlAPI"];
    ...
    {
        Producto? itemToView = null; // Inicializa la variable itemToView
