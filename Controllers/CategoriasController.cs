...
public class CategoriasController(CategoriasClientService categorias) : Controller
{
    ...
    public async Task<ActionResult> Detalle(int id)
    {
        ...
        {
            if (ex.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                return RedirectToAction("Sallir", "Auth");
        }
        return View(item);
    }
    ...
    [HttpPost]
    public async Task<ActionResult> CrearAsync(Categoria itemToCreate)
    {
    ...
    public async Task<IActionResult> EditarAsync(int id, Categoria itemToEdit)
    {
        ...
        ModelState.AddModelError("Nombre", "No hasido posible realizar la acción. Intentelo nuevamente");
        return View(itemToEdit);
    }
    ...
    public async Task<IActionResult> Eliminar(int id, bool? showError = false)
    {
        ...
        try
        {
            ...
            ViewData["ErrorMessage"] = "No ha sido posible realizar la acción. Intentelo nuevamente";
        }
        catch (HttpRequestException ex)
        {
            if (ex.StatusCode == System.Net.HttpStatusCode.Unauthorized)
